...
Per-iteration output:
    • Console summary (suggested point, bubble count, safe set size)
    • 2-D contour plot of the true basin with:
        - safe set shaded in green
        - all evaluated points shown as numbered markers
        - trajectory arrows connecting successive evaluations
        - current best point highlighted
        - true optimum marked with a star
    • Final summary plot (bubble count over time + safe-set growth)

Usage:
    python vial_optimizer_2d.py            # simulation mode (default)

Install:
    pip install safeopt GPy matplotlib numpy
"""
...
# 30 × 30 = 900 grid points — fine enough for clear 2-D plots

# Warm-start: must be a known-safe point
INITIAL_SAFE_PARAMS = {
    "nozzle_height_mm":   15.0,
    "fluid_velocity_mLs":  2.0,
}
INITIAL_BUBBLE_COUNT = 6      # observed bubble count at the seed

# True optimum of the synthetic basin (unknown to SafeOpt)
H_OPT = 15.0   # mm
V_OPT =  2.5   # mL/s
...
# 2.  REWARD HELPER
# ══════════════════════════════════════════════════════════════
# SafeOpt maximises J(a) ≥ J_min.
# We define  J(a) = SAFETY_THRESHOLD − bubble_count(a)
# so that  J ≥ 0  ⟺  bubble_count ≤ SAFETY_THRESHOLD.
...
# ══════════════════════════════════════════════════════════════
# 4.  SYNTHETIC OBJECTIVE  (convex quadratic bowl)
# ══════════════════════════════════════════════════════════════
#
...
# Pre-compute the true basin on a dense grid for plotting
...
# Safety contour level in *bubble* space = SAFETY_THRESHOLD
# In reward space the safe boundary is reward = 0,
# i.e. bubble_count = SAFETY_THRESHOLD.
...
def plot_iteration(iteration: int,
    ...
    """
    Draw a 2-D contour plot of the true basin with:
      - unsafe region shaded red, safe region shaded green
      - safety boundary contour (bubble_count = SAFETY_THRESHOLD)
      - numbered evaluation markers joined by trajectory arrows
      - current best highlighted in red
      - true optimum marked with a ★
      - newly evaluated point circled
    """
    fig, ax = plt.subplots(figsize=(7, 5.5))

    # ── Background: true bubble-count contours ────────────────
    ...
    # ── Safety boundary (bubble_count = SAFETY_THRESHOLD) ─────
...
for iteration in range(1, MAX_ITERATIONS + 1):
    ...
    # ── 10a. Ask SafeOpt for the next candidate ───────────────
    try:
        x_next = opt.optimize()
...
# ── Panel 1: bubble count over time ──────────────────────────
ax = axes[0]
...
# ── Panel 3: full 2-D trajectory on true basin ───────────────
ax = axes[2]
cf = ax.contourf(HH_dense, VV_dense, BB_dense,
